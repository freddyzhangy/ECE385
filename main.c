...
{
	// Implement this function
		unsigned char temp_message[128];
		unsigned char temp_key[128];

		int i = 0, j = 0;

		for(i = 0; i < 16; i++){
			temp_message[i] = msg_enc[2 * i];
		}
		...
		for(i = 0; i < 9; i++){
			...
			InvShiftRows(temp_message);

			for(j = 0; j < 16; j++)
							temp_message[j] = InvSubBytes(temp_message[j]);

			InvMixColumns(temp_message);
			AddRoundKey(temp_message, key_schedule, (i + 1));
		...
		for(i = 0; i < 4; i++){
			...
			msg_dec[i] = (temp_message[i * 4] >> 24) + (temp_message[i * 4 + 1] >> 16) + (temp_message[i * 4 + 2] >> 8) + (temp_message[i * 4 + 3]);
